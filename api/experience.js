...
const GITHUB_EXPERIENCE_ID = process.env.GITHUB_EXPERIENCE_ID;
...
export default async function handler({ method }, response) {
  try {
    ...
    if (!GITHUB_EXPERIENCE_ID) return response.status(500).json({ error: 'Gist ID is missing' });

    const { data } = await github({
      method,
      url: GITHUB_EXPERIENCE_ID || '',
    });
